...
/** URL the client owner visits to grant Drive+YouTube access. `state` carries the client id. */
...
function parseFolderId(input) {
  ...
  // e.g. https://drive.google.com/drive/folders/1Isijx0...?usp=drive_link
...
async function uploadToDrive({ tokens, filePath, name, folderId }) {
  ...
  // Make it link-viewable so it can be shared.
  try {
...
/**
 * Upload a video to YouTube as a (private-by-default) public Short.
 * madeForKids is forced false ("not made for kids").
 */
...
module.exports = { getAuthUrl, exchangeCode, uploadToDrive, uploadToYouTube, SCOPES };