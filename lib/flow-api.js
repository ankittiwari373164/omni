...
// Move the mouse along a few randomized points + a small scroll, so reCAPTCHA
// Enterprise observes human-like pointer entropy (a big input to the score).
...
// Parse "http://user:pass@host:port" / "socks5://host:port" / "host:port"
// into Playwright's proxy option, or return undefined.
function parseProxy(proxyUrl) {
  if (!proxyUrl || typeof proxyUrl !== "string" || !proxyUrl.trim()) return undefined;
  try {
    ...
    return { server: proxyUrl.trim() };
  }
}

async function generateVideo({ cookies, prompt, imagePath = null, aspectRatio = "portrait", duration = 8, outDir, baseName, proxy = null, onLog = () => {} }) {
  // Per-account proxy (passed in) takes priority; otherwise fall back to the
  // global PROXY_SERVER env so a single proxy can cover all accounts.
  const proxyOpt = parseProxy(proxy) || (process.env.PROXY_SERVER ? {
    server: process.env.PROXY_SERVER,
    username: process.env.PROXY_USERNAME || undefined,
    password: process.env.PROXY_PASSWORD || undefined
  } : undefined);
  onLog(proxyOpt ? `🌐 Launching browser on labs.google via proxy ${proxyOpt.server}...` : "🌐 Launching browser on labs.google...");

  // Real Google Chrome (channel:"chrome") has a far cleaner fingerprint than
  // bundled Chromium for reCAPTCHA Enterprise. ignoreDefaultArgs drops the
  // "--enable-automation" flag Playwright normally injects (a hard bot tell).
  const launchOpts = {
    headless: false,
    proxy: proxyOpt,
  ...
  try {
    ...
    onLog("🟢 Using real Google Chrome (better fingerprint)");
  } catch (e) {
    onLog(`⚠️ Real Chrome unavailable (${e.message.split("\n")[0]}) — falling back to Chromium`);
  ...
  await context.addInitScript(() => {
    ...
    // Plugins + mimeTypes (headless has none — a strong bot signal)
    const fakePlugins = [
    ...
    // permissions.query — headless returns "denied" abnormally
    ...
    // WebGL vendor/renderer — headless GPU strings scream "automation"
  ...
  try {
    ...
    // Appears on EU/UK/non-US proxy IPs and physically overlays the settings
    // button, so every click is intercepted. Click accept, then force-hide any
    // leftover bar so it can never block pointer events.
    try {
      const cookieBtn = page.locator(
        'button:has-text("Accept all"), button:has-text("Accept"), button:has-text("I agree"), button:has-text("Agree"), button:has-text("Got it"), #glue-cookie-notification-bar-1 button'
      ).first();
      if (await cookieBtn.isVisible({ timeout: 4000 })) {
        await cookieBtn.click({ timeout: 5000 }).catch(() => {});
        onLog("Accepted cookie banner");
      }
    } catch {}
    // Belt-and-suspenders: nuke any cookie/consent bar still in the DOM.
    try {
      await page.evaluate(() => {
        document.querySelectorAll(
          '[id*="cookie-notification-bar"], .glue-cookie-notification-bar, [class*="cookie-notification"]'
    ...
    // Warm up first: let reCAPTCHA Enterprise observe human-like pointer/scroll
    // activity BEFORE we call execute(), so the token scores higher.
    onLog("🖱️  Warming up (human activity for reCAPTCHA score)...");
    await humanMouse(page, 7);
    await human(page, 800, 1600);
    ...
    // ── STEP 4: Dismiss ALL modals BEFORE typing — they wipe the composer ──
    onLog("Dismissing any modals before typing...");
    for (let i = 0; i < 5; i++) {
      try {
        const modal = page.locator('button:has-text("Got it"), button:has-text("Dismiss"), button:has-text("Close"), button:has-text("OK"), button:has-text("Continue")').first();
        if (await modal.isVisible({ timeout: 1500 })) {
          await modal.click();
          onLog("Dismissed modal");
    ...
    // Verify prompt is actually in the box
    const typed = await composer.evaluate(e => e.value || e.innerText || e.textContent || "").catch(() => "");
    onLog(`Prompt in box: ${typed.length} chars`);
    if (typed.length < 10) {
      // Try fill as fallback
      await composer.fill(prompt).catch(() => {});
      await human(page, 500, 1000);
      const typed2 = await composer.evaluate(e => e.value || e.innerText || e.textContent || "").catch(() => "");
      onLog(`After fill: ${typed2.length} chars`);
    }
    ...
    while (!capturedOpName && Date.now() < deadline) {
      ...
      if (ticks % 3 === 0) {
        ...
        const pageState = await page.evaluate(() => {
          ...
          // Very specific error card detection — must have BOTH signals
          const hasErrorCard = (body.includes("unusual activity") && body.includes("Help Center")) ||
            (body.includes("We noticed some unusual") && body.includes("Please visit"));
          return {
            queued: body.includes("Queued") || body.includes("queued"),
            generating: body.includes("Generating") || /\d+%/.test(body),
            failed: hasErrorCard,
            hasVideo: !!document.querySelector("video[src]"),
            // Dump a larger slice for debugging
        ...
        if (ticks === 1) onLog(`PAGE_DUMP: ${pageState.snippet}`);
        if (pageState.failed) {
          onLog(`🚨 Error card detected: unusual activity`);
          // Take a screenshot and keep polling briefly to confirm
          try {
            const errSnap = path.join(outDir, `debug_error_${baseName}.png`);
            await page.screenshot({ path: errSnap });
          } catch {}
          throw new Error("Flow blocked: unusual activity");
        }
