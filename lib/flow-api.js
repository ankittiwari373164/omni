...
async function getTokenFromBrowser(cookies, onLog) {
  ...
  try {
    ...
    if (!oauthToken) {
      // No OAuth token found — return just the recaptcha token and use cookie auth
      onLog("⚠️  OAuth token not found in page, will use cookie auth");
      return { recaptchaToken: token, oauthToken: null };
    }

    onLog(`✅ OAuth token captured`);
    return { recaptchaToken: token, oauthToken };
...
async function generateVideo({ cookies, prompt, imagePath = null, aspectRatio = "portrait", duration = 8, outDir, baseName, onLog = () => {} }) {
  ...
  const { recaptchaToken, oauthToken } = await getTokenFromBrowser(cookies, onLog);
  ...
  // Add OAuth Bearer token if we captured it — this is what the API actually needs
  if (oauthToken) {
    ...
    onLog("⚠️  No OAuth token — using cookie auth only (may get 401)");
  }
