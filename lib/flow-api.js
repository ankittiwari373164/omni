...
async function solveRecaptcha(cookies) {
  ...
  if (service === "anticaptcha") {
    ...
    // API: POST https://v1.captchasolv.com/solve (single endpoint, not createTask/getTaskResult)
    ...
    if (res.errorId && res.errorId !== 0)
      throw new Error(`CaptchaSolv error: ${res.errorDescription || JSON.stringify(res)}`);
