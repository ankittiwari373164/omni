...
async function solveRecaptcha(cookies) {
  ...
  if (service === "anticaptcha") {
    ...
    // API: POST https://v1.captchasolv.com/solve
    ...
    const raw = await fetch("https://v1.captchasolv.com/solve", {
      ...
      headers: {
        ...
        "api-key": apiKey,          // some services require key as header
    ...
    if (res.errorId && res.errorId !== 0)
      throw new Error(`CaptchaSolv error ${res.errorId}: ${res.errorDescription || JSON.stringify(res)}`);

    const token = res.solution?.token || res.solution?.gRecaptchaResponse || res.token || res.gRecaptchaResponse;
    if (!token) throw new Error(`CaptchaSolv: no token in response: ${JSON.stringify(res)}`);
    console.log("[flow-api] reCAPTCHA solved ✓");
    return token;
