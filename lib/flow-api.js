...
async function solveRecaptcha(cookies) {
  ...
  if (service === "anticaptcha") {
    ...
    // CaptchaSolv uses anti-captcha compatible API format
    const create = await fetch("https://api.captchasolv.com/createTask", {
      ...
      // Try alternate base URL if first fails
      const r2 = await fetch("https://app.captchasolv.com/api/createTask", {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          clientKey: apiKey,
          task: { type: "RecaptchaV3EnterpriseTaskProxyless", websiteURL: FLOW_SITE_URL, websiteKey: FLOW_SITE_KEY, pageAction: "generate", minScore: 0.9 }
        })
      });
      return r2.json();
    });
    if (create.errorId && create.errorId !== 0) throw new Error(`CaptchaSolv create error: ${create.errorDescription || JSON.stringify(create)}`);
    const taskId = create.taskId;
    if (!taskId) throw new Error(`CaptchaSolv: no taskId in response: ${JSON.stringify(create)}`);
    for (let i = 0; i < 60; i++) {
      await new Promise(r => setTimeout(r, 3000));
      ...
      if (res.errorId && res.errorId !== 0) throw new Error(`CaptchaSolv poll error: ${res.errorDescription}`);
    }
