...
async function solveRecaptcha(cookies, siteKey) {
  ...
  if (service === "anticaptcha") {
    ...
    // Free trial at nextcaptcha.com — no credit card needed
    // ReCaptchaV3HSTaskProxyLess = v3 High Score proxyless (correct for Enterprise sites)
    ...
    if (create.errorId && create.errorId !== 0) throw new Error(`NextCaptcha error: ${create.errorDescription} | ${JSON.stringify(create)}`);
    const taskId = create.taskId;
    if (!taskId) throw new Error(`NextCaptcha: no taskId in response: ${JSON.stringify(create)}`);
    for (let i = 0; i < 60; i++) {
      await new Promise(r => setTimeout(r, 3000));
      ...
      if (res.status === "ready") { console.log("[flow-api] reCAPTCHA solved ✓"); return res.solution.gRecaptchaResponse; }
      if (res.errorId && res.errorId !== 0) throw new Error(`NextCaptcha poll error: ${res.errorDescription}`);
    }
