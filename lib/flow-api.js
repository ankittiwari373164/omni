...
async function generateVideo({ cookies, prompt, imagePath = null, aspectRatio = "portrait", duration = 8, outDir, baseName, onLog = () => {} }) {
  ...
  try {
    ...
    // Dismiss ANY modal that might be blocking the UI (including onboarding)
    for (let i = 0; i < 3; i++) {
      try {
        const modal = page.locator('button:has-text("Got it"), button:has-text("Dismiss"), button:has-text("Close"), button:has-text("OK")').first();
        if (await modal.isVisible({ timeout: 2000 })) {
          await modal.click();
          onLog("Dismissed blocking modal");
