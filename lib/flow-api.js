...
async function generateVideo({ cookies, prompt, imagePath = null, aspectRatio = "portrait", duration = 8, outDir, baseName, onLog = () => {} }) {
  ...
  try {
    ...
    while (!capturedOpName && Date.now() < deadline) {
      ...
      ticks++;
      if (ticks % 5 === 0) {
        onLog(`   still waiting... (${Math.round((Date.now() - (deadline - 3*60*1000))/1000)}s, oauth=${!!oauthToken}, body=${!!capturedBody})`);
        // If we have oauth but no op yet, the generation may have already fired before our route was set up
        // Check page for generation activity
        const activity = await page.evaluate(() => {
          const body = document.body.innerText;
          return body.includes("Queued") || body.includes("Generating") || !!document.querySelector("video[src]");
        });
        if (activity) onLog("   Generation activity detected on page!");
      }
    }
