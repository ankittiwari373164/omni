...
async function generatePrompt({ businessName, businessDetails, topic, hook, styleKey, styleInstruction, promptSample, splitParts, chatLink }) {
  ...
  const sys = `You write short prompts for an AI text-to-video tool. Keep everything generic
and brand-safe: no real people, brands, politics, violence, or adult content. Plain US English.`;

  const user = splitParts
    ? `Topic: ${topic}${businessDetails ? ` (brand: ${businessName} — ${businessDetails})` : ""}

Write a 20-second video as TWO parts. Each part has 2-3 short scenes. Same subject,
setting and style in both; PART 2 continues where PART 1 ends. Output ONLY this:

PART 1:
Scene 1: <one short sentence>
Scene 2: <one short sentence>
PART 2:
Scene 1: <one short sentence>
Scene 2: <one short sentence>`
    : `Topic: ${topic}${businessDetails ? ` (brand: ${businessName} — ${businessDetails})` : ""}

Write a 10-second video as 3-5 short scenes, same subject and style throughout.
Output ONLY this (no paragraph, no extra text):

Scene 1: <one short sentence>
Scene 2: <one short sentence>
Scene 3: <one short sentence>`;
