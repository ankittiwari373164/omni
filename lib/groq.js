...
//   -> { result: "<text>" }
// ============================================================================
// ALL TEXT GENERATION NOW USES PERSISTENT ChatGPT PROFILE
// These Groq functions are deprecated - they won't be called anymore
// ============================================================================

// The persistent ChatGPT profile is used in server.js:
// - generatePromptViaProfile() for prompts
// - generateImagesViaProfile() for images
// - generateCalendar() uses Groq (see below) OR return error

const groq = {
  chat: {
    completions: {
      async create({ messages = [], response_format, _chatLink } = {}) {
        throw new Error("Groq functions deprecated — all text generation uses persistent ChatGPT profile");
      }
    }
  }
};
const MODEL = "chatgpt-profile";   // not actually used
...
async function generatePrompt({ businessName, businessDetails, topic, hook, styleKey, styleInstruction, promptSample, splitParts, parts, chatLink, voiceoverEnabled = true, voiceoverLanguage = "Hindi/Hinglish" }) {
  throw new Error("generatePrompt via Groq is not available — use persistent ChatGPT profile instead");
}
...
async function generateCalendar({ businessName, businessDetails, days = 30, startDate, videosPerDay = 1, chatLink }) {
  throw new Error("generateCalendar via Groq is not available — add topics manually to the calendar");
}

async function generateCalendarOLD({ businessName, businessDetails, days = 30, startDate, videosPerDay = 1, chatLink }) {
...
async function generateNewsPromptOLD({ businessName, businessDetails, title, summary, splitParts, parts, chatLink, voiceoverEnabled, voiceoverLanguage, skipThemeDistillation = false }) {
...
module.exports = { generatePrompt, enhancePrompt, splitPromptParts, extractPartImages, sanitizePrompt, generateNewsPrompt, generateCalendar, generateYouTubeMeta, generateVoiceScript, promptStyles };
