 ...
 */
// ── Calendar generation uses the REAL Groq API (openai/gpt-oss-120b) — NOT the
// ChatGPT browser flow. This is a plain HTTPS call (fetch), so it needs no
// browser at all: works on the Hostinger dashboard (no Playwright/Chromium
// there) and is much faster than driving a browser. Only calendar generation
// uses this; prompts and meta content still go through the logged-out ChatGPT
// browser flow in callChatGPT, unchanged.
const GROQ_API_KEY = process.env.GROQ_API_KEY || "";
const GROQ_API_MODEL = process.env.GROQ_API_MODEL || "openai/gpt-oss-120b";

async function generateCalendar({ businessName, businessDetails, days = 30, startDate, videosPerDay = 1, chatLink }) {
  if (!GROQ_API_KEY) throw new Error("GROQ_API_KEY not set — required for calendar generation");
...
- ${days} items, days 1..${days}, all unique.`;
  ...
  if (!res.ok) {
    ...
    throw new Error(`Groq API HTTP ${res.status}: ${errBody.slice(0, 200)}`);
  }
