...
async function callChatGPT(promptText, chatLink) {
  // CHANGED: Use persistent ChatGPT profile instead of ngrok
  ...
  while (Date.now() < deadline) {
    ...
    if (messages.length > 1) {
      ...
      if (text && text.length > 30 && !text.includes("(typing)") && !text.includes("…")) {
        response = text.trim();
        break;
      }
    }
  ...
  if (!response) throw new Error("ChatGPT did not respond");
  return response;
}
