...
async function callChatGPT(promptText, chatLink) {
  ...
  // Increase timeout to 180 seconds (3 minutes) for ChatGPT to generate
  ...
  try {
    ...
    if (e.name === "AbortError") {
      throw new Error("ChatGPT generation timeout (took over 3 minutes)");
    }
    throw e;
  }
}
...
async function generateImage(promptText, chatLink) {
  ...
  // Increase timeout to 180 seconds (3 minutes) for image generation
  const controller = new AbortController();
  const timeoutId = setTimeout(() => controller.abort(), 180000);
  ...
  try {
    ...
    clearTimeout(timeoutId);
    if (e.name === "AbortError") {
      throw new Error("Image generation timeout (took over 3 minutes)");
    }
    throw e;
  }
}
