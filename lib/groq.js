...
async function callChatGPT(promptText, chatLink) {
  // Use persistent ChatGPT profile. CRITICAL: Always start a NEW chat so that
  // one client's prompt NEVER contaminates another client's (no context bleed).
  const { chromium } = require("playwright");
  const p = require("path");
  const f = require("fs");

  const profileDir = p.join(__dirname, "..", "sessions", "chatgpt");
  if (!f.existsSync(profileDir)) throw new Error("ChatGPT profile not found — run: node save-sessions.js");

  let browser;
  try {
    browser = await chromium.launchPersistentContext(profileDir, { headless: true, channel: "chrome" });
  } catch (e) {
    browser = await chromium.launchPersistentContext(profileDir, { headless: true });
  }

  const page = browser.pages()[0] || await browser.newPage();

  try {
    // ALWAYS open a brand-new chat (never reuse a conversation). This is the
    // fix for prompts mixing between clients — each call is fully isolated.
    await page.goto("https://chatgpt.com/", { waitUntil: "domcontentloaded", timeout: 30000 }).catch(() => {});
    await page.waitForTimeout(3000);

    // Click "New chat" to guarantee a clean context (belt-and-suspenders).
    try {
      const newChatBtn = page.locator('a[href="/"], button:has-text("New chat"), [data-testid="new-chat-button"]').first();
    ...
    // Find the input box
    let input = await page.locator("textarea").first().waitFor({ timeout: 10000 }).catch(() => null);
    if (!input) input = await page.locator("[contenteditable='true']").first().waitFor({ timeout: 5000 }).catch(() => null);
    if (!input) { await browser.close(); throw new Error("ChatGPT input not found"); }
    ...
    while (Date.now() < deadline) {
      const messages = await page.locator('[data-message-author-role="assistant"], [class*="message"]').all().catch(() => []);
      if (messages.length > 0) {
        const lastMsg = messages[messages.length - 1];
        const text = await lastMsg.innerText().catch(() => "");
        if (text && text.length > 20) {
          // Wait for response to stabilize (stop growing) = generation complete
          if (text.length === lastLength) {
            stableCount++;
            if (stableCount >= 3) { response = text.trim(); break; }
          } else {
            stableCount = 0;
    ...
    await browser.close();
    if (!response) throw new Error("ChatGPT did not respond in time");
    return response;
  } catch (e) {
...
- ${days} items, days 1..${days}, all unique.`;
  ...
  for (let topicIndex = 0; topicIndex < (parsed.items || []).length; topicIndex++) {
    const dayOffset = Math.floor(topicIndex / videosPerDayNum);
    const slotIndex = topicIndex % videosPerDayNum;
