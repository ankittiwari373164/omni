...
function houseFormatBlock(n) {
  const blocks = [];
  for (let k = 0; k < n; k++) blocks.push(partBlock(k));
  return blocks.join("\n\n");
}

async function generatePrompt({ businessName, businessDetails, topic, hook, styleKey, styleInstruction, promptSample, splitParts, parts, chatLink }) {
 ...
 */
async function enhancePrompt({ userPrompt, businessName, businessDetails, splitParts, parts, chatLink }) {
  const n = normParts({ parts, splitParts });
  const totalSec = n * 10;
  const markers = Array.from({ length: n }, (_, k) => `"PART ${k + 1}"`).join(", ");
 ...
 */
async function generateNewsPrompt({ businessName, businessDetails, title, summary, splitParts, parts, chatLink }) {
