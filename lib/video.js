...
/**
 * Concatenate multiple clips into one (re-encoded to a common size first so
 * two Flow clips with slightly different dims join cleanly). Returns out path.
 */
async function concatParts(parts, outDir, baseName) {
  fs.mkdirSync(outDir, { recursive: true });
  const valid = parts.filter(p => p && fs.existsSync(p));
  if (valid.length === 0) throw new Error("concatParts: no valid input clips");
  if (valid.length === 1) {
    const only = path.join(outDir, `${baseName}_joined.mp4`);
    fs.copyFileSync(valid[0], only);
    return only;
  }
  // Use the first clip's dimensions as the target.
  const m0 = await probe(valid[0]);
  const W = m0.width || 1080, H = m0.height || 1920;

  const normed = [];
  for (let i = 0; i < valid.length; i++) {
    const n = path.join(outDir, `${baseName}_part${i}.mp4`);
    await normalize(valid[i], n, W, H);
    normed.push(n);
  }
  const listFile = path.join(outDir, `${baseName}_parts.txt`);
  fs.writeFileSync(listFile, normed.map(p => `file '${path.resolve(p)}'`).join("\n") + "\n");

  const joined = path.join(outDir, `${baseName}_joined.mp4`);
  await run(FFMPEG, ["-y", "-f", "concat", "-safe", "0", "-i", listFile, "-c", "copy", joined]);

  [...normed, listFile].forEach(f => { try { fs.unlinkSync(f); } catch {} });
  return joined;
}

module.exports = { compose, applyFrame, probe, concatParts };