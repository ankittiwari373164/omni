...
(async () => {
  ...
  if (IMAGE_PATH) {
    ...
    try {
      ...
      if (clipResult.ok) {
        const chipSel = '[class*="attachment" i], [class*="chip" i], [class*="thumbnail" i], [class*="preview" i], [role="textbox"] img, img[src^="blob:"], img[src^="data:"]';
        let chip = 0;
        for (let tryN = 0; tryN < 2 && chip === 0; tryN++) {
          await promptBox.click({ force: true });
          await page.waitForTimeout(400);
          await page.keyboard.press("Control+V");
          await page.waitForTimeout(2800);
          chip = await page.locator(chipSel).count();
          if (chip === 0 && tryN === 0) console.log("Image chip not detected — re-pasting once");
        }
        if (chip > 0) {
          console.log("✅ Image chip detected in prompt box");
        } else {
          console.log("⚠️  Image chip NOT detected after 2 paste attempts — generating without confirmed image");
          await snap(page, "no_image_chip");
        }
      } else {
