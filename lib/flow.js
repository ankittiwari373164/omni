...
(async () => {
  ...
  // STRATEGY 0 (PRIMARY, most reliable): download the actual <video> file via
  // the page's own authenticated fetch. Doesn't need any bounding box / UI
  // layout — works even when the card isn't visually positioned. We read the
  // REAL resolved src from the <video> element (a googleusercontent/blob URL),
  // then fetch it inside the page so cookies + redirects are handled natively.
  try {
    console.log("Download via in-page fetch of <video> src...");
    // Give the new clip a moment to attach its real src.
    await page.waitForTimeout(3000);
    const bytes = await page.evaluate(async (np) => {
      const vids = Array.from(document.querySelectorAll('video'));
      // Prefer the freshly generated clip by name; else the first getMediaUrl video.
      let v = np ? vids.find(x => (x.src || "").includes(np) || (x.currentSrc || "").includes(np)) : null;
      if (!v) v = vids.find(x => /getMediaUrl|googleusercontent|blob:/.test(x.src || x.currentSrc || ""));
      if (!v) v = vids[0];
      if (!v) return null;
    ...
    if (Array.isArray(bytes) && bytes.length > 10000) {
      fs.writeFileSync(outPath, Buffer.from(bytes));
      console.log(\`✅ Downloaded (in-page video src): \${fileName} (\${(bytes.length/1024/1024).toFixed(1)} MB)\`);
      console.log("__VIDEO__:" + fileName);
      downloaded = true;
    } else {
      console.log("in-page video-src fetch unavailable:", bytes && bytes.err ? bytes.err : "no bytes");
    }
  } catch (e) { console.log("in-page video-src download failed:", e.message.split("\\n")[0]); }
