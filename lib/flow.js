...
// If FLOW_PROJECT_URL is set, open that exact project directly (clean slate).
// Otherwise fall back to the home page + "Try Omni now" carousel.
const PROJECT_URL = process.env.FLOW_PROJECT_URL || "";
...
// variable pause — humans don't act on fixed intervals (slower defaults)
const human = (page, min = 900, max = 2600) => page.waitForTimeout(rand(min, max));

// type slowly with inconsistent per-key speed, thinking pauses, and typo-and-correct
async function humanType(page, text) {
  for (let i = 0; i < text.length; i++) {
    // ~3% chance of a typo we immediately fix, like a real typist
    if (Math.random() < 0.03 && /[a-z ]/i.test(text[i])) {
      const wrong = "abcdefghijklmnopqrstuvwxyz"[rand(0, 25)];
      await page.keyboard.type(wrong, { delay: rand(90, 220) });
      await page.waitForTimeout(rand(250, 600));
      await page.keyboard.press("Backspace");
      await page.waitForTimeout(rand(150, 400));
    }
    // slower per-key (humans ~40-80 wpm => ~120-260ms/char)
    await page.keyboard.type(text[i], { delay: rand(90, 240) });
    // thinking pauses after spaces/punctuation, more often and longer
    if (/[.,! ]/.test(text[i]) && Math.random() < 0.30) await page.waitForTimeout(rand(400, 1400));
    // occasional longer mid-sentence pause, like re-reading
    if (Math.random() < 0.04) await page.waitForTimeout(rand(800, 2200));
  }
}

// move the mouse in to the target over several steps, settle, then click — like a hand
async function humanClick(page, locator) {
  try {
    const box = await locator.boundingBox();
    if (box) {
      // approach in two hops with a pause, as a person reaching for a button
      await page.mouse.move(box.x + box.width * Math.random(), box.y - rand(20, 60), { steps: rand(6, 12) });
      await page.waitForTimeout(rand(150, 450));
      await page.mouse.move(box.x + box.width * (0.3 + Math.random() * 0.4), box.y + box.height * (0.3 + Math.random() * 0.4), { steps: rand(4, 9) });
...
// Idle pointer entropy + scroll — feeds reCAPTCHA Enterprise human-behaviour signals.
async function humanMouse(page, moves = 5) {
  try {
    let x = rand(200, 900), y = rand(150, 600);
    for (let i = 0; i < moves; i++) {
      const nx = Math.min(1270, Math.max(5, x + rand(-180, 180)));
      const ny = Math.min(790, Math.max(5, y + rand(-140, 140)));
      await page.mouse.move(nx, ny, { steps: rand(8, 20) });
      x = nx; y = ny;
      await page.waitForTimeout(rand(80, 260));
    }
    if (Math.random() < 0.7) { await page.mouse.wheel(0, rand(120, 500)); await page.waitForTimeout(rand(200, 500)); }
  } catch {}
}
...
(async () => {
  ...
  // Real Google Chrome (channel:"chrome") fingerprints far better than bundled
  // Chromium for reCAPTCHA Enterprise. ignoreDefaultArgs drops Playwright's
  // "--enable-automation" flag (a hard bot tell). Fall back to Chromium if Chrome
  ...
  const launchOpts = {
    ...
    args: [
      ...
      "--no-first-run", "--no-default-browser-check", "--disable-infobars", "--password-store=basic"
    ]
  };
  let browser;
  try {
    browser = await chromium.launch({ ...launchOpts, channel: "chrome" });
    console.log("🟢 Using real Google Chrome (better fingerprint)");
  } catch (e) {
    console.log("⚠️ Real Chrome unavailable — falling back to Chromium:", e.message.split("\\n")[0]);
  ...
  if (PROJECT_URL) {
    // Direct to a specific (clean) project — avoids reusing the shared default
    // project that accumulates stale images / error cards between runs.
    console.log("Opening project directly:", PROJECT_URL);
    try {
      await page.goto(PROJECT_URL, { waitUntil: "domcontentloaded", timeout: NAV_TIMEOUT });
    } catch (e) {
      console.log("⚠️ Proxy too slow to load project (" + e.message.split("\\n")[0] + ") — exiting so the worker rotates to the next proxy");
    ...
    try {
      ...
      console.log("⚠️ Proxy too slow to load Flow (" + e.message.split("\\n")[0] + ") — exiting so the worker rotates to the next proxy");
      await browser.close(); process.exit(1);
    }
    await page.waitForLoadState("load", { timeout: NAV_TIMEOUT }).catch(() => {});
  ...
  // ── Dismiss the Google cookie consent bar ──
  // It overlays the settings + generate buttons and intercepts every click.
  // Accept it, then force-remove any leftover bar so it can't block pointers.
  try {
    const cookieBtn = page.locator('button:has-text("Accept all"), button:has-text("Accept"), button:has-text("I agree"), button:has-text("Agree"), #glue-cookie-notification-bar-1 button').first();
  ...
  try {
    await page.evaluate(() => {
      document.querySelectorAll('[id*="cookie-notification-bar"], .glue-cookie-notification-bar, [class*="cookie-notification"]')
        .forEach(el => { el.style.display = "none"; el.style.pointerEvents = "none"; el.remove(); });
    });
  } catch {}
  ...
  for (let i = 0; i < menuCount; i++) {
    ...
    if (txt?.includes("Video")) {
      await allMenuBtns.nth(i).click({ timeout: 15000 }).catch(e => console.log("Settings click issue:", e.message.split("\\n")[0]));
  ...
  if (SKIP_IMAGE) {
    ...
    console.log("Attaching image via Add Media menu:", IMAGE_PATH);
    let attached = false;
    try {
      // "Add Media" opens a SUBMENU (Upload / From images / ...). The file
      // chooser only fires when we click the Upload item inside that menu.
      const addBtn = page.locator('button:has-text("Add Media"), button[aria-label*="add media" i], button:has-text("add_2")').first();
      await humanClick(page, addBtn).catch(() => {});
      await human(page, 600, 1200);

      // Click an "Upload" option in the opened menu while intercepting the chooser.
      const uploadItem = page.locator(
        '[role="menuitem"]:has-text("Upload"), [role="menuitem"]:has-text("upload"), ' +
        'text=Upload from computer, text=Upload media, text=Uploads, li:has-text("Upload")'
      ).first();

      const [chooser] = await Promise.all([
        page.waitForEvent("filechooser", { timeout: 8000 }).catch(() => null),
        (async () => { try { if (await uploadItem.count()) await uploadItem.click({ timeout: 4000 }); } catch {} })()
      ]);
      ...
      if (chooser) {
        ...
        // Fallback 1: a hidden <input type=file> exposed directly
      ...
      // Fallback 2: clipboard paste straight into the prompt box (older Flow builds)
      if (!attached) {
        try {
          await page.keyboard.press("Escape").catch(() => {});
          const mime = IMAGE_PATH.match(/\\.png$/i) ? "image/png" :
                       IMAGE_PATH.match(/\\.webp$/i) ? "image/webp" :
                       IMAGE_PATH.match(/\\.gif$/i) ? "image/gif" : "image/jpeg";
          const b64 = fs.readFileSync(IMAGE_PATH).toString("base64");
          await promptBox.click({ force: true });
          await page.waitForTimeout(400);
          const ok = await page.evaluate(async ({ b64, mime }) => {
            try {
          ...
          if (ok) {
            ...
            attached = true;
            console.log("Attached image via clipboard paste fallback");
          }
        } catch (e) { console.log("Clipboard fallback failed:", e.message); }
      }

      const chipSel = '[class*="attachment" i], [class*="chip" i], [class*="thumbnail" i], [class*="preview" i], img[src^="blob:"], img[src^="data:"], [role="textbox"] img';
  ...
  await human(page, 3000, 7000);
  // Pointer entropy right before generate, so Flow's invisible reCAPTCHA scores us human
  await humanMouse(page, 6);
  await human(page, 600, 1400);
