...
async function waitForCompletion(page, maxSeconds = 300) {
  console.log("Polling for completion...");
  for (let s = 0; s < maxSeconds; s += 5) {
    await page.waitForTimeout(5000);
    const state = await page.evaluate(() => {
      const body = document.body.innerText;
      const vid  = document.querySelector('video[src*="getMediaUrl"]');
      return {
    ...
    console.log(\`  [\${s+5}s] queued=\${state.isQueued} gen=\${state.isGenerating} video=\${state.hasVideo}\`);
    if (state.isPolicy) { console.log("→ Policy violation"); return { status: "policy" }; }
    if (state.videoSrc && !state.isQueued && !state.isGenerating)
      { console.log("→ Success!"); return { status: "success", src: state.videoSrc }; }
  }
...
(async () => {
  ...
  const result = await waitForCompletion(page, 300);
  ...
  try {
    const card = page.locator('[class*="gouBtR"]').first();
